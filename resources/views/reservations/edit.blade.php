...
@section('dashboard-content')
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-white shadow-lg rounded-lg px-8 pt-6 pb-8 mb-4">
            <h2 class="text-3xl font-bold mb-6 text-gray-800">Modifier la Réservation</h2>
            ...
            <form action="{{ route('reservations.update', $reservation->id) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <select name="user_id" id="client_id"
                        ...
                        @foreach ($clients as $client)
                            <option value="{{ $client->id }}" {{ (old('user_id', $reservation->user_id) == $client->id) ? 'selected' : '' }}>
                                {{ $client->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6 flex flex-wrap -mx-2">
                    <div class="w-full md:w-1/2 px-2 mb-4 md:mb-0">
                        ...
                        <input type="date" name="date_arrivee" id="date_arrivee" value="{{ old('date_arrivee', $reservation->date_arrivee) }}"
                    ...
                    <div class="w-full md:w-1/2 px-2">
                        ...
                        <input type="date" name="date_departe" id="date_depart" value="{{ old('date_departe', $reservation->date_departe) }}"
                            class="block w-full border @error('date_departe') border-red-500 @else border-gray-300 @enderror rounded-lg px-4 py-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400">
                        @error('date_departe')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
