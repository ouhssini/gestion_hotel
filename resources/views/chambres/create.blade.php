    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
            <div class="px-8 py-6 bg-gradient-to-r from-blue-600 to-blue-700">
                <h2 class="text-2xl font-bold text-white">Ajouter une Nouvelle Chambre</h2>
                <p class="mt-1 text-blue-100">Remplissez les informations ci-dessous pour créer une nouvelle chambre</p>
            </div>
            ...
            <form method="POST" action="{{ route('chambres.store') }}" class="px-8 py-6 space-y-6">
                @csrf
                <div class="grid grid-cols-1 gap-6">
                    <div class="space-y-2">
                        ...
                        <select id="type" name="type_id" required class="block w-full px-4 py-3 rounded-lg border @error('type') border-red-300 @else border-gray-300 @enderror focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out">
                            @foreach($types as $type)
                                <option value="{{ $type->id }}" {{ old('type') == $type->id ? 'selected' : '' }}>{{ $type->titre }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea id="description" name="description" rows="4" required 
                            ...
                            placeholder="Entrez la description de la chambre...">{{ old('description') }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <input type="number" id="superficier" name="superficie" required 
                                    ...
                                    placeholder="Entrez la superficie..." value="{{ old('superficier') }}">
                        ...
                        <div class="space-y-2">
                            ...
                            <select id="etage" name="etage" required 
                                ...
                                <option value="RDC" {{ old('etage') == 'RDC' ? 'selected' : '' }}>Rez-de-chaussée</option>
                                <option value="1" {{ old('etage') == '1' ? 'selected' : '' }}>1er Étage</option>
                                <option value="2" {{ old('etage') == '2' ? 'selected' : '' }}>2ème Étage</option>
                                <option value="3" {{ old('etage') == '3' ? 'selected' : '' }}>3ème Étage</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <input type="number" id="prix" name="prix" step="0.01" required 
                                ...
                                placeholder="Entrez le prix..." value="{{ old('prix') }}">
